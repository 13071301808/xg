...
public class WxEnterSpecialProcess extends ProcessFunction<String, JSONObject> {
    static Logger logger = LoggerFactory.getLogger(WxEnterSpecialProcess.class);
    @Override
    public void processElement(String value, Context context, Collector<JSONObject> collector) throws Exception {
        try {
            ...
            if (eventData != null && EventParseUtil.isEvent(data, "enterspecial")) {
                // 日志流自带
                ...
                String special_id  = eventData.getString("special_id");
                String special_name  = eventData.getString("special_name");
                ...
                wxGoodsDetailPageResult.put(ModelUtil.humpToUnderline("special_id"), special_id);
                wxGoodsDetailPageResult.put(ModelUtil.humpToUnderline("special_name"), special_name);
                wxGoodsDetailPageResult.put(ModelUtil.humpToUnderline("source"), source);
                wxGoodsDetailPageResult.put("event", "enterspecial");

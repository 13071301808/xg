package com.yishou.bigdata.realtime.dw.common.generation;

/**
 * @date: 2023/6/16
 * @author: yangshibiao
 * @desc: 小时表数据监控（使用当前小时的专场日来监控）
 * 监控1 => 主键重复
 * 监控2 => 数据条数为0
 */
public class HourTableMonitoV1Generation {

    public static void main(String[] args) {

        // 表名 ☆☆☆☆☆
        String tableName = "yishou_daily.dwt_user_offline_basic_feature";

        // 主键字段名  ☆☆☆☆☆
        String primaryKey = "user_id";

        // 运行的环境（dli、dws）
        String dataSource = "dli";

        // 报错的作业名即可   ☆☆☆☆☆
        String jobName = "dwt_user_offline_basic_feature_用户离线基础特征";

        // 运行环境（dev：告警二群，prod：大数据告警群， analyst：分析师作业监控群）
        String env = "prod";

        // 是否阻断（Y|N, 默认为Y，阻断）
        String intercept = "N";

        StringBuilder sql = new StringBuilder();

        sql.append("-q \"");

        sql.append(" select * ");
        sql.append(" from ( ");
        sql.append(" select ");
        sql.append(" concat('主键字段为" + primaryKey + "，其中主键重复数为', count(*)) as 主键存在重复 ");
        sql.append(" from ( ");
        sql.append(" select ");
        sql.append(" " + primaryKey + ", count(*) as cnt ");
        sql.append(" from " + tableName + " ");
        sql.append(" where dt = ${current_special_date} ");
        sql.append(" group by " + primaryKey + " ");
        sql.append(" having cnt > 1 ");
        sql.append(" ) ");
        sql.append(" having count(*) > 0 ");
        sql.append(" ) as alarm_1 ");

        sql.append(" full join ( ");
        sql.append(" select ");
        sql.append(" '" + tableName + "表数据条数为0' as 数据条数异常 ");
        sql.append(" from ( ");
        sql.append(" SELECT ");
        sql.append(" count(*) as cnt ");
        sql.append(" from " + tableName + " ");
        sql.append(" where dt = ${current_special_date} ");
        sql.append(" ) ");
        sql.append(" where cnt = 0 ");
        sql.append(" ) as alarm_2 ");
        sql.append(" on 1 = 1 ");

        sql.append("\"");

        sql.append(" -t " + dataSource);
        sql.append(" -n " + jobName);
        sql.append(" -d 数据异常");
        sql.append(" -g " + env);
        sql.append(" -b " + intercept);

        System.out.println(sql);

    }

}
